    ...
    TrtInferNode() : Node("trt_infer_node")
    {
        ...
        if (file_path_.empty() || trt_path_.empty()) {
            RCLCPP_ERROR(this->get_logger(), 
                ...
                "  ros2 run your_pkg trt_infer_node --ros-args "
...
private:
    ...
    void runPipeline()
    {
        ...
        RCLCPP_INFO(this->get_logger(), "All done. Results wrote to ../../infer_results/trt.txt");
    }
...
int main(int argc, char *argv[])
{
    ...
    // 如果节点构造时出错(比如参数为空)，不会退出，这里仍可spin一下
    // 也可以提前判断后直接shutdown
